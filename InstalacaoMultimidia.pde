...
class Bola {
  ...
  void draw() {
    ...
    float alvo = map(sliderForma.valor, 0, 2, 3, 15);
    formaInterpolada += (alvo - formaInterpolada) * 0.1f;
    int nVertices = (int) constrain(formaInterpolada, 3, 15);

    float r = tamanhoBola / 2;

    beginShape();
    for (int i = 0; i < nVertices; i++) {
      float ang = TWO_PI * i / nVertices;
      float px = pos.x + cos(ang) * r;
      float py = pos.y + sin(ang) * r;
      vertex(px, py);
    }
    endShape(CLOSE);
  }
